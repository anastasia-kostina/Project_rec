...
class MainRecommender:
    ...
    fake_id: int
        Идентификатор, которым заменялись редкие объекты
    """
    ...
    @staticmethod
    def fit(user_item_matrix, n_factors=150, regularization=0.0001, iterations=5, num_threads=4):
        """Обучает ALS"""
    ...
    def get_similar_items_recommendation(self, user, N=5):
        ...
        top_user_items = self.top_purchases[self.top_purchases['user_id'] == user]
        top_user_items = top_user_items.head(N)

        res = top_user_items['item_id'].apply(lambda x: self._get_similar_item(x).tolist()).to_numpy().tolist()
    ...
    def get_similar_users_recommendation(self, user, N=5):
        ...
        res = []
        users = self.model.similar_users(self.userid_to_id[user], N=N + 1)[0]
        users_id = [self.id_to_userid[us] for us in users]
        users_id = users_id[1:]

        for u in users_id:
            rec = self.get_own_recommendations(user, N=1)
            res.extend(rec)
            if rec in res:
                j = 1
                while True:
                    j = j + 1
                    rec_new = self.get_own_recommendations(user, N=j)[-1]
                    if rec_new not in res:
                        res.pop()
                        res.append(rec_new)
                        break
